...
} from '../../services/foodLookUpService';
...
describe('searchFoods', () => {
  ...
  beforeEach(() => {
    ...
    deps = mockDeps();
    deps.axios.get.mockResolvedValue({ data: { foods: [{ fdcId: 1 }] } });
  });

  it('calls the USDA search endpoint with the query, api key, and default page size', async () => {
    await searchFoods('chicken breast', 25, deps);

    expect(deps.axios.get).toHaveBeenCalledWith(
  ...
  it('respects a custom pageSize when provided', async () => {
    await searchFoods('chicken breast', 5, deps);
  ...
  it('returns response.data unwrapped, not the full axios response', async () => {
    const result = await searchFoods('chicken breast', 25, deps);
    expect(result).toEqual({ foods: [{ fdcId: 1 }] });
  });

  it('propagates the rejection when the USDA request fails', async () => {
    deps.axios.get.mockRejectedValue(
      new Error('Request failed with status code 400'),
    );

    await expect(searchFoods('chicken breast', 25, deps)).rejects.toThrow(
...
describe('getFoodDetails', () => {
  ...
  it('calls the USDA food detail search endpoint with the api key', async () => {
    await getFoodDetails(12345, deps);

    expect(deps.axios.get).toHaveBeenCalledWith(
      expect.stringContaining('/food/12345'),
      {
        params: expect.objectContaining({
          api_key: 'test-api-key',
        }),
  ...
  it('returns response.data unwrapped, not the full axios response', async () => {
    const result = await getFoodDetails(12345, deps);
  ...
  it('propagates the rejection when the USDA request fails', async () => {
    ...
    await expect(getFoodDetails(999999, deps)).rejects.toThrow(
...
describe('extractSearchResults', () => {
  ...
  it('fills missing brandName/dataType with null', () => {
    const data = {
      foods: [
        {
          fdcId: 100,
